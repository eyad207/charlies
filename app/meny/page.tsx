...
export default function Meny() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Track search bar visibility - synchronized with header
  ...
  return (
    <div className='bg-gradient-to-b from-gray-50 to-white min-h-screen'>
      ...
      <motion.section
        initial={{ y: 0 }}
        animate={{
          y: isSearchVisible ? 0 : -400,
        }}
        transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }}
        className='fixed left-0 right-0 pt-7 tb-6 md:pt-14 md:tb-6 rounded-3xl bg-white/95 backdrop-blur-md top-[150px] z-[45] shadow-lg border-b-2 border-gray-100'
      >
        <div className='container mx-auto px-4 '>
          <div className='flex flex-col md:flex-row gap-3 md:gap-4 items-center'>
            ...
            <div className='relative flex-1 w-full'>
              ...
              <input
                ...
                className='w-full pl-12 pr-4 py-3 md:py-4 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-[#FDB714] focus:ring-2 focus:ring-[#FDB714]/20 transition-all text-gray-900 cursor-text shadow-sm hover:border-[#FDB714]/50'
              />
            </div>

            {/* Category Filter */}
            <div className='flex items-center gap-3 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide'>
              <Filter className='text-gray-600 flex-shrink-0' size={18} />
              {categories.map((category) => (
                <motion.button
                  ...
                  className={`px-3 py-2 md:px-5 md:py-2.5 rounded-xl font-bold whitespace-nowrap transition-all cursor-pointer ${
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className='mt-3 md:mt-4 text-gray-600 font-medium'
          >
            Viser{' '}
